    ...
    <main className="page-shell">
      <section className="hero" aria-labelledby="portal-title">
        ...
        {auth ? (auth.user.role === 'recruiter' ? (
          ...
          <AdminDashboard user={auth.user} onSignOut={signOut} />
        ) : (
...
function AdminDashboard({ user, onSignOut }) {
  ...
  async function loadStats() {
    ...
    const token = localStorage.getItem('placementPortalToken');
    try {
      const response = await fetch(`${apiUrl}/admin/stats`, { headers: { Authorization: `Bearer ${token}` } });
      const data = await response.json();
  ...
  return (
    <section className="auth-card admin-dashboard" aria-label="Admin dashboard">
      <p className="eyebrow">Phase 7 · Admin dashboard</p>
      <h2>Welcome, {user.name}</h2>
      <p>Placement analytics across students, recruiters, jobs, and applications.</p>
      {status && <p className={stats ? 'muted' : 'form-error'} role="status">{status}</p>}
      {stats && <>
        <section className="stats-grid">
          <div className="stat-card"><span className="stat-value">{stats.totals.students}</span><span className="stat-label">Students</span></div>
          <div className="stat-card"><span className="stat-value">{stats.totals.recruiters}</span><span className="stat-label">Recruiters</span></div>
          <div className="stat-card"><span className="stat-value">{stats.totals.jobs}</span><span className="stat-label">Jobs posted</span></div>
          <div className="stat-card"><span className="stat-value">{stats.totals.activeJobs}</span><span className="stat-label">Active jobs</span></div>
          <div className="stat-card"><span className="stat-value">{stats.totals.applications}</span><span className="stat-label">Applications</span></div>
          <div className="stat-card"><span className="stat-value">{stats.placementRate}%</span><span className="stat-label">Placement rate</span></div>
        </section>
        <section className="applications-section" aria-labelledby="status-heading">
          <h2 id="status-heading">Application status breakdown</h2>
          <div className="application-row"><span>Applied</span><span className="status-pill">{stats.statusCounts.applied}</span></div>
          <div className="application-row"><span>Shortlisted</span><span className="status-pill">{stats.statusCounts.shortlisted}</span></div>
          <div className="application-row"><span>Rejected</span><span className="status-pill">{stats.statusCounts.rejected}</span></div>
          <div className="application-row"><span>Selected</span><span className="status-pill">{stats.statusCounts.selected}</span></div>
        </section>
        <section className="applications-section" aria-labelledby="skills-heading">
          <h2 id="skills-heading">Most in-demand skills</h2>
          {stats.topSkills.length ? (
            <div className="job-grid skills-grid">
              {stats.topSkills.map((entry) => (
                <div className="application-row" key={entry.skill}><span>{entry.skill}</span><span className="status-pill">{entry.count}</span></div>
              ))}
            </div>
          ) : <p className="muted">No skills recorded on posted jobs yet.</p>}
        </section>
        <button type="button" className="secondary-button" onClick={loadStats}>Refresh dashboard</button>
      </>}
      <button type="button" className="secondary-button" onClick={onSignOut}>Sign out</button>
    </section>
  );
